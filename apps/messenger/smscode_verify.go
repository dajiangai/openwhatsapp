...
// Error .
func (p *SMSCodeVerifyResp) Error() error {
	if p.Status == "ok" {
		return nil
	} else {
...
func (app *Messenger) MakeSMSVerify(code string) string {
	...
	model := SMSCodeVerify{
		...
		Rc:       "0",
		Lg:       app.Language,
		Lc:       app.Country, // device.GetCountry(),
